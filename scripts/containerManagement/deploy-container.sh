...
NAMESPACE="admin-ui"
CONFIG_DIR="k8s"
SECRET_NAME="admin-ui-secrets" # pragma: allowlist secret
IMAGE_NAME="admin-ui-service"
IMAGE_TAG="latest"
...
print_separator "="
echo -e "${CYAN}🟢 Building Angular Docker image: ${FULL_IMAGE_NAME} (inside Minikube Docker daemon)${NC}"
print_separator '-'
...
print_separator "="
echo -e "${CYAN}📦 Deploying Admin UI Service container...${NC}"
print_separator "-"

kubectl apply -f "${CONFIG_DIR}/deployment.yaml"

print_separator "="
echo -e "${CYAN}🌐 Exposing Admin UI Service via ClusterIP Service...${NC}"
print_separator "-"
...
kubectl wait --namespace="$NAMESPACE" \
  ...
  --selector=app=admin-ui-service \
  --timeout=90s

print_separator "-"
print_status "ok" "Admin UI Service is up and running in namespace '$NAMESPACE'."
...
if grep -q "admin-ui.local" /etc/hosts; then
  echo -e "${YELLOW}🔄 Updating /etc/hosts for admin-ui.local...${NC}"
  sed -i "/admin-ui.local/d" /etc/hosts
else
  echo -e "${YELLOW}➕ Adding admin-ui.local to /etc/hosts...${NC}"
fi
echo "$MINIKUBE_IP admin-ui.local" | tee -a /etc/hosts
print_status "ok" "/etc/hosts updated with admin-ui.local pointing to $MINIKUBE_IP"

print_separator "="
echo -e "${GREEN}🌍 You can now access your app at: http://sous-chef-proxy.local${NC}"

POD_NAME=$(kubectl get pods -n "$NAMESPACE" -l app=admin-ui-service -o jsonpath="{.items[0].metadata.name}")
SERVICE_JSON=$(kubectl get svc admin-ui-service -n "$NAMESPACE" -o json)
SERVICE_IP=$(echo "$SERVICE_JSON" | jq -r '.spec.clusterIP')
SERVICE_PORT=$(echo "$SERVICE_JSON" | jq -r '.spec.ports[0].port')
INGRESS_HOSTS=$(kubectl get httproute -n "$NAMESPACE" -o jsonpath='{.items[*].spec.hostnames[*]}' | tr ' ' '\n' | sort -u | paste -sd ',' -)
...
echo "  Health Check: http://sous-chef-proxy.local/api/v1/admin-ui/health"
echo "  Readiness Check: http://sous-chef-proxy.local/api/v1/admin-ui/ready"
echo "  Liveness Check: http://sous-chef-proxy.local/api/v1/admin-ui/live"
print_separator "="
