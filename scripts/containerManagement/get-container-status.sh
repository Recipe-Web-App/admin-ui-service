...
echo "📊 Admin UI Service Status Dashboard"
print_separator "="
...
if kubectl get deployment admin-ui-service -n "$NAMESPACE" >/dev/null 2>&1; then
    kubectl get deployment admin-ui-service -n "$NAMESPACE"

    READY_REPLICAS=$(kubectl get deployment admin-ui-service -n "$NAMESPACE" -o jsonpath='{.status.readyReplicas}' 2>/dev/null || echo "0")
    DESIRED_REPLICAS=$(kubectl get deployment admin-ui-service -n "$NAMESPACE" -o jsonpath='{.spec.replicas}' 2>/dev/null || echo "0")
...
if kubectl get pods -n "$NAMESPACE" -l app=admin-ui-service >/dev/null 2>&1; then
    kubectl get pods -n "$NAMESPACE" -l app=admin-ui-service

    POD_NAME=$(kubectl get pods -n "$NAMESPACE" -l app=admin-ui-service -o jsonpath="{.items[0].metadata.name}" 2>/dev/null || echo "")
    if [ -n "$POD_NAME" ]; then
        echo ""
...
if kubectl get service admin-ui-service -n "$NAMESPACE" >/dev/null 2>&1; then
    kubectl get service admin-ui-service -n "$NAMESPACE"
...
if kubectl get ingress admin-ui-ingress -n "$NAMESPACE" >/dev/null 2>&1; then
    kubectl get ingress admin-ui-ingress -n "$NAMESPACE"

    INGRESS_IP=$(kubectl get ingress admin-ui-ingress -n "$NAMESPACE" -o jsonpath='{.status.loadBalancer.ingress[0].ip}' 2>/dev/null || echo "")
...
if kubectl get configmap admin-ui-config -n "$NAMESPACE" >/dev/null 2>&1; then
    print_status "ok" "ConfigMap exists"
    CONFIG_KEYS=$(kubectl get configmap admin-ui-config -n "$NAMESPACE" -o jsonpath='{.data}' | jq -r 'keys[]' 2>/dev/null | wc -l || echo "0")
    echo "   🔑 Configuration keys: $CONFIG_KEYS"
else
    print_status "error" "ConfigMap not found"
fi

if kubectl get secret admin-ui-secrets -n "$NAMESPACE" >/dev/null 2>&1; then
    print_status "ok" "Secret exists"
    SECRET_KEYS=$(kubectl get secret admin-ui-secrets -n "$NAMESPACE" -o jsonpath='{.data}' | jq -r 'keys[]' 2>/dev/null | wc -l || echo "0")
...
if kubectl get networkpolicy admin-ui-network-policy -n "$NAMESPACE" >/dev/null 2>&1; then
...
if kubectl top pods -n "$NAMESPACE" --no-headers 2>/dev/null | grep -q admin-ui-service; then
    echo -e "${BLUE}  Current resource usage:${NC}"
    kubectl top pods -n "$NAMESPACE" --no-headers | grep admin-ui-service
...
if kubectl get service admin-ui-service -n "$NAMESPACE" >/dev/null 2>&1; then
    SERVICE_IP=$(kubectl get service admin-ui-service -n "$NAMESPACE" -o jsonpath='{.spec.clusterIP}' 2>/dev/null || echo "unknown")
    SERVICE_PORT=$(kubectl get service admin-ui-service -n "$NAMESPACE" -o jsonpath='{.spec.ports[0].port}' 2>/dev/null || echo "unknown")
