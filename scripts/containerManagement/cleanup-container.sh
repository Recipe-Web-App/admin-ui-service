...
echo "🧹 Cleaning up Admin UI Service resources..."
print_separator "="
...
kubectl delete deployment admin-ui-service -n "$NAMESPACE" --ignore-not-found
print_status "ok" "Deployment deletion completed"

print_separator
echo -e "${CYAN}🌐 Deleting service...${NC}"
kubectl delete service admin-ui-service -n "$NAMESPACE" --ignore-not-found
print_status "ok" "Service deletion completed"

print_separator
echo -e "${CYAN}🔒 Deleting network policy...${NC}"
kubectl delete networkpolicy admin-ui-network-policy -n "$NAMESPACE" --ignore-not-found
print_status "ok" "Network policy deletion completed"

print_separator
echo -e "${CYAN}⚙️  Deleting configmap...${NC}"
kubectl delete configmap admin-ui-config -n "$NAMESPACE" --ignore-not-found
print_status "ok" "ConfigMap deletion completed"

print_separator
echo -e "${CYAN}🔐 Deleting secret...${NC}"
kubectl delete secret admin-ui-secrets -n "$NAMESPACE" --ignore-not-found
...
if grep -q "admin-ui.local" /etc/hosts; then
  ...
  print_status "ok" "Removed admin-ui.local from /etc/hosts"
else
